...
int fournisseur::getPoidsProduit() { return poidsProduit; }
...
void fournisseur::setPoidsProduit(int Poids) { poidsProduit = Poids; }
...
QSqlQueryModel* fournisseur::afficher() {
    QSqlQueryModel *model = new QSqlQueryModel();
      model->setQuery("SELECT nom, adresse, code, produit, "
                      ...
                      "TO_CHAR(poids) AS poids "
                      "FROM fournisseur");

      // Setting the header data for each column
      ...
      model->setHeaderData(6, Qt::Horizontal, QObject::tr("Poids"));  // Ensure poids is in header
...
bool fournisseur::modifier() {
    ...
    query.prepare("UPDATE fournisseur SET nom = :nom, adresse = :adresse, produit = :produit, "
                  "contact = :contact, quantite = :quantite, poids = :poids "
                  "WHERE code = :code");
...
fournisseur fournisseur::getConstatDetails(int code){
    ...
    if (query.exec()) {
        if (query.next()) {
            ...
            f.setPoidsProduit(query.value(6).toInt());
...
bool fournisseur::updatePoidsProduit(QString code, int poids)
{
    // Check if the code exists in the database first
    QSqlQuery checkQuery;
    checkQuery.prepare("SELECT COUNT(*) FROM fournisseur WHERE code = :code");
    checkQuery.bindValue(":code", code);

    if (!checkQuery.exec()) {
        qDebug() << "Failed to check for code:" << checkQuery.lastError();
        return false;
    }
    ...
    if (count == 0) {
        qDebug() << "Code not found in database.";
        return false;
    }

    // If code exists, proceed with the update
    QSqlQuery query;
    query.prepare("UPDATE fournisseur SET poids = :poids WHERE code = :code");
    query.bindValue(":poids", poidsProduit);
    query.bindValue(":code", code);
    ...
    if (!query.exec()) {
        qDebug() << "Failed to update poids:" << query.lastError().text();
        return false;
    }

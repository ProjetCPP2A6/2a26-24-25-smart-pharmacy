...
ordonnanceUI *ordonnanceWindow = nullptr;

void fournisseurUI::on_pushButton_30_clicked()
{
    // Vérifier si l'instance de ordonnanceUI existe déjà
        if (ordonnanceWindow == nullptr) {
            // Créer l'instance seulement si elle n'existe pas encore
            ordonnanceWindow = new ordonnanceUI(this);  // 'this' comme parent
        }

        // Vérifier si la fenêtre est déjà visible avant de l'afficher
        if (!ordonnanceWindow->isVisible()) {
            // Cacher l'interface fournisseurUI avant d'ouvrir ordonnanceUI
            this->hide();  // Cache l'interface actuelle (fournisseurUI)
            ordonnanceWindow->show();  // Affiche l'interface ordonnanceUI
        }
}
